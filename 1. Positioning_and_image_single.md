...
- Before/After metrics (time-to-feature, first-pass success, p95 run duration, resource usage per run; optionally error rate, throughput, or p95 latency).
...
Example:
> Manual intake was consuming 12 hours/week. Built a LangGraph decision flow behind typed FastAPI endpoints and n8n triggers, then codified with run management (state timeline, retries, diagnostics). First‑pass success 92%, p95 run 2.1s, ~3.1k tokens/run. Evidence: run viewer and before/after panel. Watch the 90‑second demo → [link]
...
- Abstract: Most “AI automations” ship without a way to see what actually happened. Demonstrating a minimal run viewer: run list, state timeline, and step diagnostics — and why this reduces failures and clarifies outcomes. Screenshot includes first‑pass success, p95 run duration, and resource usage.
...
- Abstract: LangGraph classifies emails, extracts entities, and proposes CRM entries with one‑click human approval. First‑pass success and p95 run stats included; full run viewer snapshot.
...
- Abstract: OCR → field extraction → policy validation → ERP API write. First‑pass success and p95 run duration shown; errors reduced materially. One‑minute run viewer walk‑through.
