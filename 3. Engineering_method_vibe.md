...
- **Metrics:** Latency, accuracy, token cost, and pass rate.
...
- **Observability:** Logs, metrics, and traces attached to PRs.
- **Scorecard:** Each PR gets a numeric health score (code + eval + outcomes).

Business-first KPIs for SMB-facing reporting:
- Hours saved per week and monthly savings (hours_saved × hourly_cost × 4.3)
- Cost per task (before → after) and error-rate reduction
- Turnaround/response time and cash-cycle improvements

Engineering telemetry (internal):
- p95/p99 latency, token cost/run, error codes, and retry counts

Example metrics:
- Hours saved/week ≥ target; monthly savings trend up and to the right
- Error rate reduction ≥ 50% in target workflows; turnaround time −30%+
- Internal SLOs: p95 latency < 2.5s; cost/run within budget envelope; eval pass rate ≥ 90%
...
- **Dashboards:** Visualize project health (tests, evals, coverage, cost, latency).
...
| **p95 Latency** | 95th percentile latency — the time within which 95% of requests complete. |
