# WebUI Deployment — VPS with Traefik

This guides you to deploy the Vite-built WebUI as a static SPA served by Nginx behind Traefik. The image is built and published by GitHub Actions to GHCR, and the VPS pulls it on deploy.
...
# 2) Compose up the WebUI (pulls GHCR image; Traefik labels configured)
cd deploy
$env:UI_HOST = "ui.example.com"; docker compose -f ui.compose.yml --env-file .env up -d  # deploy file remains in deploy/ as ui.compose.yml
```

After a few seconds, Traefik should route https://$UI_HOST to the WebUI container.
...
## Notes
- The WebUI container serves static files and proxies same-origin requests to backend services:
  - `/api/` -> `http://api:8000/`
  - `/auth/` -> `http://auth:8000/`
...
Note: The WebUI proxies /api and /auth to container DNS names (api, auth). For end-to-end local calls, either run API/Auth on the same docker network with matching compose files, or change the Nginx proxy targets to host.docker.internal:PORT for local-only testing.

## Troubleshooting
- 404 for images: ensure `/images/Flovify-logo.png` exists in repo root; favicons under `images/favicon/` should include `favicon-32x32.png` referenced by `webui/index.html`.
