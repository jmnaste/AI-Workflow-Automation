...
- Top-level folders only
	...
	- `auth/` — Auth service (Python). Owns `auth` schema and Alembic migrations.
...
- Rules
	...
	- Migrations: prefer per-service, transactional migrations owned by the service (Alembic for Python). Avoid shared, manual DB state.
